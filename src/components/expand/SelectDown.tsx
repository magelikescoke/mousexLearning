...
export default function SelectDown(props: SDProps) {
// const SelectDown = (props: SDProps, ref?) => {
    // 是否显示其他项的标记
                ...
                (props.form.getFieldValue(props.selectName) || []).map((x, index) => {
                    for (let i = 0; i < props.options.length; i++) {
                        if (x === props.options[i]['value']) {
                            return <div
                                key={'popText' + index}>{index + 1}. {props.options[i]['content'] ? props.options[i]['content'] : props.options[i]['title']}</div>;
                        }
                    }
                })
            }
    ...
    return (
        <>
            <Popover content={popContent} visible={(optArr || []).length !== 0 && visible}
                ...
                <Form.Item name={props.selectName} label={props.selectTitle}>
                    <Select
                        mode={'multiple'}
                        maxTagTextLength={2}
                        // style={{ width: 240 }}
...
// export default React.forwardRef(SelectDown);